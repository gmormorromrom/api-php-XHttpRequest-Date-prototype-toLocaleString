...
<body>
    ...
    <script>
        function getCustomers() {
            let urlCustomers = "/api/customers.php"
            ...
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    var dateFR = date.toLocaleString('fr-FR', {
                        timeZone: 'Africa/Tunis'
                    });
                    document.getElementById("customersid").innerHTML += "Date and time(Fr) : " + dateFR + '<br>';
                    for (let key in objData.names) {
                        // document.getElementById("customersid").innerHTML += parseInt(key)+1+"-"+objData.names[key].firstname +'<br>';
                        var name = "";
                        for (let k in objData.names[key]) {
                            if (k == 0) {
                                name = objData.names[key][k].firstname;
                            } else if (k == 1) {
                                name += " " + objData.names[key][k].lastname;
                            }

                        }
                        document.getElementById("customersid").innerHTML += parseInt(key) + 1 + "-" + name+ '<br>';
                    }
                }
            };
